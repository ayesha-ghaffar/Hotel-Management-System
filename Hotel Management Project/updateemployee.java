...
public class updateemployee extends JFrame implements ActionListener {
    ...
    updateemployee(){
        ...
        String jobs[] = {"Reception","Housekeeping","Kitchen Staff","Room Service","Waiter/Waitress","Manager","Accountant","Chef"};
        dropdown = new JComboBox(jobs);
        dropdown.setBackground(Color.WHITE);
        dropdown.setBounds(200,330,150,30);
        add(dropdown);

        fid= new JTextField();
        fid.setBounds(200, 380, 150, 30);
        add(fid);

        fdepid= new JTextField();
        fdepid.setBounds(200, 480, 150, 30);
        add(fdepid);

        fmid= new JTextField();
        fmid.setBounds(200, 530, 150, 30);
        add(fmid);
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        String job = (String)dropdown.getSelectedItem();

       // String hdate=date.getText();
        ...
        try{
           ...
           Connection connection = c.getConnection();
            PreparedStatement pstmt = connection.prepareStatement("UPDATE STAFF SET FirstName=?,LastName=?, Age=?, Gender=?, Salary=?, Contactno=?,JobTitle=?,HireDate=?,DepID=?,ManagerID=? WHERE  StaffID=?");
